...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const paymentsResponse = await PaymentsService.getAllPayments();
        setRecentTransactions(paymentsResponse.data.slice(0, 4)); // Just take the first 4

        // Fetch recent service requests
        const purchasesResponse = await PurchasesService.getAllPurchases();
        setRecentRequests(purchasesResponse.data.slice(0, 3)); // Just take the first 3
      } catch (err: any) {
  ...
  // Extracted stats for easier access
  const platformStats = {
    ...
    totalTransactions: stats?.totalPurchases || 0,
    completionRate: stats?.completionRate || 0,
    newUsersToday: stats?.recentUsers?.length || 0,
    transactionsToday: stats?.recentPurchases?.length || 0,
  };
            ...
            <Table>
              ...
              <TableBody>
                {recentTransactions.map((tx) => {
                  ...
                  const fee = tx.amount * 0.1;

                  return (
                    <TableRow key={tx.id}>
                      <TableCell className="font-medium">
                        {tx.purchase?.buyer?.user?.name || "Unknown Buyer"}
                      </TableCell>
                      <TableCell>
                        {tx.purchase?.seller?.user?.name || "Unknown Seller"}
                      </TableCell>
                      <TableCell>${tx.amount.toFixed(2)}</TableCell>
            ...
            <Table>
              ...
              <TableBody>
                {recentRequests.map((req) => (
                  <TableRow key={req.id}>
                    <TableCell className="font-medium">
                      {req.buyer?.user?.name || "Unknown Buyer"}
                    </TableCell>
                    <TableCell>
                      {req.seller?.user?.name || "Unknown Seller"}
                    </TableCell>
                    <TableCell>
                      {req.services.map((s: any) => s.service.title).join(", ")}
                    </TableCell>
