...
const SellerServices: React.FC = () => {
  ...
  const handleToggleAvailability = async (serviceId: string) => {
    try {
      const response = await ServicesService.toggleServiceAvailability(
        serviceId
      );

      console.log("Response from toggle availability:", response);
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSaveService} disabled={submitting}>
              {submitting ? (
                <>
